...
from utils import load_data, CORA_TRAIN_RANGE, CORA_TEST_RANGE, CORA_VAL_RANGE
...
def parse_args():
    ...
    args.add_argument("--eval_period", type = int, default = 100, help = "Specify evaluation period.")

    return args.parse_args()

# TODO: More elegant wau to write accuracy in Torch?
...
if __name__ == "__main__":
    ...
    for epoch in range(args.epochs):
        ...
        if epoch % args.eval_period == 0 or epoch == args.epochs - 1:
            model.eval()
            with torch.no_grad():
                logits = model.forward(data)
                ...
                print(f"Epoch: {epoch} --- Train step and inference time: {(time.time() - start):.4f}s. --- Train accuracy: {train_acc:.4f} --- Val accuracy: {val_acc:.4f}")
        
        create_checkpoint(args.model_dir, epoch, model, optimizer)
